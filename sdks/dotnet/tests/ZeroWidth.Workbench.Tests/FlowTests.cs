using Xunit;
using ZeroWidth.Workbench.Testing;
...
public class FlowTests
{
    ...
    public FlowTests()
    {
        ...
        var testProjectDir = FindProjectDir(currentDir, "ZeroWidth.Workbench.Tests");
    ...
    private static string FindProjectDir(string startDir, string projectName)
    {
        ...
        while (current != null)
        {
            ...
            {
                return Path.Combine(current.FullName, "tests", "ZeroWidth.Workbench.Tests");
            }
    ...
    [Fact]
    public async Task AdditionTest_WorkbenchEngine_ShouldPass()
    {
        var runner = new FlowTestRunner(_flowsDir, debug: false);
